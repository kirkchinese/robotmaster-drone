...
Eigen::Vector4f UAVLinearController::execute(Eigen::VectorXf& X_des, Eigen::VectorXf& X_real)
{
    ...
    m_integral_e_z += cur_e_z; // Integral term for z
    float u1 = m_mass * m_gravity + m_mass*(m_Kdz * (cur_e_z-m_last_e_z) + m_Kpz * cur_e_z + m_Kiz * m_integral_e_z);
    m_last_e_z = cur_e_z;
    if(u1 > m_Fmax * 4)u1 = m_Fmax * 4;
    float cur_e_x = X_des[0] - X_real[0];
    m_integral_e_x += cur_e_x; // Integral term for x
    float vxd = std::min<float>(m_max_v, m_Kdx * (cur_e_x - m_last_e_x) + m_Kpx * cur_e_x + m_Kix * m_integral_e_x);
    ...
    m_integral_e_y += cur_e_y; // Integral term for y
    float vyd = std::min<float>(m_max_v, m_Kdy * (cur_e_y - m_last_e_y) + m_Kpy * cur_e_y + m_Kiy * m_integral_e_y);
    ...
    if(max_output > 1.0)
    {
        float scale = 1.0 / max_output;
        for(int i = 0;i<thrust.size();++i){
            pwm[i] = std::max<float>(0.1, pwm[i] * scale);
        }
    }
