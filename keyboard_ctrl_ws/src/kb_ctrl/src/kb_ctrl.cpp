...
{  
    ...
    front_View_suber2 = it->subscribe("airsim_node/drone_1/front_left/Scene", 1, std::bind(&KBCtrl::front_view_cb2, this,  std::placeholders::_1));
...
void KBCtrl::front_view_cb(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    cv::waitKey(2);
    //ROS_INFO_STREAM("On the position");
    if (-(last_time - ros::Time::now()).toSec() >= 1)
    {   //指定位置
        last_time = ros::Time::now();
        static int image_number = 0;
        std::string filename = "/home/misaka/drone/keyboard_ctrl_ws/data/img/left_imgsave"+ std::to_string(image_number++) + ".png";
        if (cv::imwrite(filename, cv_front_ptr->image))
        {
            ROS_INFO_STREAM("Save image success");
        }
        cv::imshow("front_left_save", cv_front_ptr->image);
        cv::waitKey(10);
    }
}

void KBCtrl::front_view_cb2(const sensor_msgs::ImageConstPtr& msg)
{   
    /*

    if ((last_time - ros::Time::now()).toSec() >= 1)
    {
        cv_front_ptr2 = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::TYPE_8UC3);
        cv::imshow("front_left_save", cv_front_ptr->image);
        cv::waitKey(50);
        last_time = ros::Time::now();
        ROS_INFO_STREAM("On the position");
    ...
    ROS_INFO_STREAM("call back exist");
    }
    */
}
