...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    no_replan_thresh_ = 0.3; /* this line is for robomaste */
    ...
    else if (target_type_ == TARGET_TYPE::PRESET_TARGET)
    {
      ...
      waypoint_sub_ = nh.subscribe("/reference_path", 1, &EGOReplanFSM::reference_pathCallback, this);   
    }
    else
  ...
  void EGOReplanFSM::reference_pathCallback(const geometry_msgs::PoseStampedPtr &msg)
  {
    if (msg->pose.position.z < -0.1)
      return;
    ...
    char buf[4096];
    sprintf(buf, "tt:%lf no_replan_thresh_ = %lf; /* this line is for robomaste */!!!!!", ros::Time::now().toSec(), no_replan_thresh_);
    std::cout << buf << msg->header.seq <<" reference!!! " <<  wps_.size() << " end_wp:"<< end_wp.transpose() << std::endl;
    for (int idx = 0; idx < wps_.size(); idx++) {
        std::cout << "[" << idx << ":" << wps_[idx].transpose() << "]";
    }
    std::cout << "wps finished" << std::endl;
