...
def angle_vec(vec1, vec2):
    ...
    vec = np.cross(vec1, vec2) / norm
    return vec, np.arccos(cost)
...
def calc_circle(pt3d):
    ...
    return origin.reshape([1, -1]), vec, yaw
'''
...
class SFM():
    def __init__(self, origin, vec, yaw, LENGTH_SAMPLE = 100):
        ...
        self.theta = np.arange(LENGTH_SAMPLE) * 2 * np.pi / LENGTH_SAMPLE - np.pi
        self.inner, self.ABC = init_circle(origin, 0.6, vec, yaw, self.theta)
        self.outer, _ = init_circle(origin, 0.78, vec, yaw, self.theta)
        
    def update_circle(self, origin, vec, yaw):
        self.origin, self.yaw, self.vec = origin, yaw, vec
        self.inner, self.ABC = init_circle(origin, 0.6, vec, yaw, self.theta)
        self.outer, _ = init_circle(origin, 0.78, vec, yaw, self.theta)
    ...
    def w2c(self, R, t): # R, t. 相机姿态 same as world
        ...
        ABC = R.dot(self.ABC)
        ABCD = np.concatenate([ABC, -origin_cam.dot(ABC)], axis = 0)
        return inner_cam, outer_cam, origin_cam, ABCD
    ...
    def w2p_v2(self, R, t):
        inner_cam, outer_cam, origin_cam, ABCD = self.w2c(R, t)
        ...
        return inner_pix, outer_pix, origin_pix, inner_img, outer_img, origin_img, ABCD
    ...
    def p2c_nos(self, pt):
        p3d = np.concatenate([pt, np.ones([pt.shape[0], 1], np.float32)], axis = 1)
        return self.CK_inv.dot(p3d.T).T
    
    def p2w_v2(self, R_, t_, pt, ABCD):
        z_pix = np.ones([pt.shape[0], 1], np.float32)
        pix_ = np.concatenate([pt, z_pix], axis = 1)
        cam_ = self.CK_inv.dot(pix_.T)
        z_pix[:, 0] = -ABCD[3, 0] / (ABCD[0:3].T.dot(cam_))
        return R_.dot(cam_ * z_pix.T + (t_ + self.CAM_T).reshape([-1, 1])).T
...
class FRAME():
    ...
    def w2c(self, sfm):
        self.inner_cam, self.outer_cam, self.origin_cam, self.ABCD = sfm.w2c(self.R_inv, -self.pos)
        
    def get_ABCD(self, sfm):
        self.ABCD = sfm.get_ABCD(self.R_inv, -self.pos)
        
    def w2p(self, sfm):
        self.inner_img, self.outer_img, self.origin_img = sfm.w2p(self.R_inv, -self.pos)
        
    def w2p_v2(self, sfm):
        self.inner_pix, self.outer_pix, self.origin_pix, self.inner_img, self.outer_img, self.origin_img, self.ABCD = \
    ...
    def p2w_v2(self, sfm):
        return sfm.p2w_v2(self.R, self.pos, self.cnt, self.ABCD)
    
    def p2w_v3(self, sfm, depth):
        return sfm.p2w(self.R, self.pos, self.cnt, depth)
